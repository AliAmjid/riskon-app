...
export class CursorAgentService {
  ...
  buildPrompt(options: TriggerRunOptions): string {
    ...
    lines.push(
      ...
      '   it prints is not empty, and that both report.md and walkthrough.md are in it.',
      '   If walkthrough.md is on the "absent" line, write it and publish again. Do not',
      '   finish without it.',
      '',
      '   walkthrough.md is how a non-technical reader learns to trust the number. It must',
      '   explain, in their language, how you turned their question into a search: what is',
      '   being chosen, what "best" means, and which rules bounded the search. The report',
      '   says what to do; the walkthrough says how you got there. They are not the same file.',
      '',
  ...
  buildFollowUpPrompt(message: string): string {
    return [
      ...
      'including a rewritten walkthrough.md if the reasoning changed.',
      '',
      'Their message:',
      message,
    ].join('\n');
